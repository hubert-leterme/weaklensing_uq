...
from .. import utils, metric
from . import optim, optim_iterators

PARAMS_ALGO = {"lambda": 1.0, "stepsize": 1.0, "g_param": 0.05}

class BaseMCALens(optim.BaseOptim):

    def __init__(
            self,
            iterator_g, iterator_ng,
            niter_per_step_g=optim_iterators.NITER_PER_STEP_G,
            niter_per_step_ng=optim_iterators.NITER_PER_STEP_NG,
            ...
            custom_init=optim.zero_init,
            set_output=lambda x: {"est": (x,)},
            update_ng_first=False, output_mode: str | None = None, **kwargs
    ):
        iterator = optim_iterators.MCAIteration(
            iterator_g, iterator_ng,
            niter_per_step_g=niter_per_step_g,
            niter_per_step_ng=niter_per_step_ng,
            update_ng_first=update_ng_first
        )
        params_algo = utils.merge_dict(params_algo_g, params_algo_ng)
        data_fidelity = utils.ModuleWrapper(data_fidelity_g, data_fidelity_ng)
        prior = utils.ModuleWrapper(prior_g, prior_ng)
    ...
    def update_metrics_fn(
            self, metrics: metric.MetricDict, X_prev, X, x_gt=None
    ):
    ...
    def forward(
        ...
        if self.output_mode is not None:
            if self.output_mode == "discard_ng":
                x, _ = utils.get_tensor_components(x) # Shape = (nimgs, nchannels, nx, ny)
            elif self.output_mode == "add_components":
                x = utils.add_tensor_components(x) # Shape = (nimgs, nchannels, nx, ny)
            else:
                raise ValueError(
    ...
    def _add_components_for_metrics(self, X):
        ...
        x = self.get_output(X)
        x_out = utils.add_tensor_components(x)
        X_out = X.copy()
