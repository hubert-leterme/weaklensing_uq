...
if [ "$#" -lt 2 ]; then
  ...
  echo "Example: $0 0 model/dir/ -cqr path/to/cqr.pt -a torch.DRUNet -s small -t yyyymmdd_hhmmss -uq -t0 yyyymmdd_hhmmss [-i 3] [-w 8]"
  exit 1
fi
...
# Command to execute
cmd=$(echo "python scripts/pnpmass.py ${path_to_test_dataset} ${model_dir} ${output_filename} ${optional_args} --seed 42 -v" | xargs)
