...
python scripts/train.py $path_to_augmented_dataset \
  ...
  -lr $lr --lr-scheduler \
  --loss $loss \
  --checkpoint-dir $checkpoint_dir/denoiser_${scale}_${loss}_meancentering_${current_date} \
  --save-freq $save_freq \
  --backup-dir $backup_dir/denoiser_${scale}_${loss}_meancentering_${current_date} \
  --path-to-csv-log $stats_dir/log_denoiser_${scale}_${loss}_meancentering_${current_date}_pe.csv --seed 42 -v
